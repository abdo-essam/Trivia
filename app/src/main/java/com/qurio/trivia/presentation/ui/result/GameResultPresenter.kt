...
/**
 * Presenter for GameResultFragment
 * Handles saving game results and navigation
 */
    ...
    fun saveGameResult(
        ...
        tryToExecute(
            execute = {
                ...
                if (stars > 0) {
                    gameResultRepository.updateUserAwards(stars)
                }

                SaveResult(coins, stars)
            },
            onSuccess = { result ->
                Log.d(TAG, "✓ Game saved successfully: coins=${result.coins}, stars=${result.stars}")
